...
/*! \file RandomNumbers_TNG.hpp
  ...
  $Id: RandomNumbers_TNG.hpp,v 1.6 2009/09/03 18:45:32 matthew Exp $
...
public:
  ...
  public:
    ...
    CDFGenerator(const Fn& fn, double lo, double hi, unsigned nbins):
      m_icdf()
    {
      std::vector< Pair > cdf;

      const double dx = (hi-lo)/(double)(nbins);
      double x = lo;
      for(unsigned ibin = 0; ibin < nbins; ibin++)
	{
	  cdf.push_back(Pair(x,fn.val(x)));
	  x+=dx;
	}

      cdf.push_back(Pair(x,1));

      RandomNumbers_TNG::GenerateInverseCDF(cdf,nbins);
      m_icdf = cdf;
    }

    double rnd(RandomNumbers_TNG* rng) { return rng->InverseCDF(m_icdf); }

  private:

    std::vector< Pair > m_icdf;
  };

  static std::string defaultFilename();
  static void GenerateInverseCDF(std::vector< Pair > &cdf, unsigned nbins = 0);
...
RandomNumbers_TNG(const std::string& state_filename, 
  ...
  RandomNumbersBase(), 
  CORE(RNGCore::DevRandom::oneUInt64(), core_options),
  m_bm_hascached(false), m_bm_cachedval(),
  m_poi_lambdaold(-1), m_poi_lambdaexp(), m_poi_lambdasrt(), m_poi_lambdalog(),
  m_bin_nold(-1), m_bin_pold(-1), m_bin_pc(), m_bin_plog(), 
  m_bin_pclog(), m_bin_en(), m_bin_oldg(),
...
RandomNumbers_TNG(const char* state_filename, const CoreOptions& core_options):
  RandomNumbersBase(), CORE(RNGCore::DevRandom::oneUInt64(), core_options),
  m_bm_hascached(false), m_bm_cachedval(),
  m_poi_lambdaold(-1), m_poi_lambdaexp(), m_poi_lambdasrt(), m_poi_lambdalog(),
  m_bin_nold(-1), m_bin_pold(-1), m_bin_pc(), m_bin_plog(), 
  m_bin_pclog(), m_bin_en(), m_bin_oldg(),
 ...
 */
template<typename CORE> void RandomNumbers_TNG<CORE>::readState()
{
