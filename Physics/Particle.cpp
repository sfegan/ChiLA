...
bool Particle::PropagateBField(double dist, const Vec3D& B)
{
  ...
  static const double E0 = Constants::cgs_eV * Constants::cgs_c * Constants::cgs_c * 1e-8;
  double wDc = 1./ Constants::cgs_c  // 1/sec / c
             * charge                // q
...
bool Particle::PropagateFreeToSphere(const Vec3D& center, double radius, 
				     ...
				     bool time_reversal_ok)
{
  ...
  switch(ip)
    ...
    case IP_PREVIOUS:
      if(!time_reversal_ok)return false;
      else if(t2<0)time=t2;
    ...
    case IP_EARLIEST:
      if((time_reversal_ok)||(t1>=0))time=t1;
      else return false;

    case IP_LATEST:
      if((time_reversal_ok)||(t2>=0))time=t2;
      else return false;
    };
...
bool Particle::InitializePDGData(const std::string& filename, bool reinitialize)
{
  ...
  for(std::vector<std::string>::iterator i=m_lines.begin(); i!=m_lines.end(); i++)
      ...
      while(str_mcids >> id)
	{
	  ...
	  particle->mcid         = id;
	  particle->name         = name;
	  particle->mass         = mass         * Constants::cgs_eV*1e9;
	  particle->mass_err_lo  = mass_err_lo  * Constants::cgs_eV*1e9;
	  particle->mass_err_hi  = mass_err_hi  * Constants::cgs_eV*1e9;
	  particle->width        = width        * Constants::cgs_eV*1e9;
	  particle->width_err_lo = width_err_lo * Constants::cgs_eV*1e9;
	  particle->width_err_hi = width_err_hi * Constants::cgs_eV*1e9;
	  particle->charge       = charge;
	  
	  s_pdgdata[id]=particle;
	  s_pdgdata_name_q[std::make_pair<std::string, int>(name,charge)]=
	    particle;
	}
    }
...
std::ostream& operator << (std::ostream& stream, const Physics::Particle::PDGData& p)
{  
  std::ostringstream s;
...
std::ostream& operator << (std::ostream& stream, const Physics::Particle& p)
{
  std::ostringstream s;
  s << p.Position() << ' ' 
