/*! \file make_egy_tables.cpp

  Program to make energy lookup tables from stage2 file
  ...
  $Id: make_egy_tables.cpp,v 3.61 2009/10/01 17:53:13 matthew Exp $
...
int main(int argc, char** argv)
{
  ...
  std::string output_file = "egy_tables.h5";
  options.findWithValue("o", output_file, 
			"Set the output file name."); 

  VSEnergyLTLibraryVisitor::configure(options);
  ...
  try
    {
      ...
      VSEnergyLTLibraryVisitor* egylt_visitor =
	new VSEnergyLTLibraryVisitor;

      VSEventDataDispatcher* dispatcher = 
	new VSEventDataDispatcher(egylt_visitor);
      
      dispatcher->processFiles(file_list);  
      
      egylt_visitor->createLibrary();
      egylt_visitor->save(writer);
