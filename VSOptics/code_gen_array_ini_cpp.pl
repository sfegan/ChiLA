...
while($member = shift @members)
  {
    ...
    printf("%-10s VSOArrayParameters::sCanonical%-30s = %s;\n",$type,$member,$val);
  }
...
void VSOArrayParameters::setCanonicalValuesFromOptions(VSOptions& options)
{
  std::string filename;
  if(options.find("APZero","Set all canonical values to zero")
     != VSOptions::FS_NOT_FOUND)zeroCanonicalValues();
  if(options.find("APReset","Reset all canonical values to compiled defaults")
     != VSOptions::FS_NOT_FOUND)resetCanonicalValues();
  if(options.findWithValue("APLoad",filename,"Load canonical values from file")
     == VSOptions::FS_FOUND)
    {
      VSOArrayParameters param;
      if(!param.readFromArrayINIFile(filename))
        {
          std::cerr << "Could not read parameters from: " << filename
                    << std::endl;
          assert(0);
        }
      setCanonicalValuesFromArrayParameters(param);
    }
END
...
while($member = shift @members)
  {
    print "  options.findWithValue(std::string(\"AP",$member,"\"),sCanonical",
      $member,",array_ini_comment[",$count,"]);\n";
    $count++;
  }
...
print << 'END';
  if(options.findWithValue("APSave",filename,"Save canonical values to file") 
     == VSOptions::FS_FOUND)
    {
      VSOArrayParameters param(true);
      param.writeToArrayINIFile(filename);
    }
}

void VSOArrayParameters::printOptions(std::ostream& stream)
{
  stream << "  -APZero                             Zero default array parameters" << std::endl;
  stream << "  -APReset                            Reset default array parameters" << std::endl;
  stream << "  -APLoad FILENAME                    Load default array parameters from file" << std::endl;
  stream << "  -APSave FILENAME                    Save default array parameters to file" << std::endl;
END
...
while($member = shift @members)
  {
    my $blab = $member." <".shift(@types).">";
    printf("  stream << \"  -AP%-32s See documentation\" << std::endl;\n",
	   $blab);
  }
...
bool VSOArrayParameters::readFromDatabase(VSDatabase* db, uint32_t optics_id)
{
  std::string collection = std::string(scCollection)
    + std::string("[")
  ...
  parameter_table.retrieveParameterSet(collection, parameters);
...
while($member = shift @members)
  {
    print("  if(parameters.find(\"",$member,"\") != parameters.end())\n");
    print("  { VSDataConverter::fromString(",$member,",parameters[\"",$member,"\"]); nfound++; }\n");
  }
...
print << 'END';
  ...
  return(nfound == NARRAYINIITEMS);
}

void VSOArrayParameters::writeToDatabase(VSDatabase* db,
...
while($member = shift @members)
  {
    print("  parameters[\"",$member,"\"]=VSDataConverter::toString(",$member,");\n");
  }
...
print << 'END';
  int nstored = parameter_table.storeParameterSet(collection, parameters);
  assert(nstored = NARRAYINIITEMS);
}

void VSOArrayParameters::createSimulationParametersTable(VSDatabase* db)
{
  VSDBParameterTable parameter_table(db);
